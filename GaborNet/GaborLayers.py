...
class GaborConv2d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size):
        ...
        self.weight = torch.empty(out_channels, in_channels, kernel_size[0], kernel_size[1], requires_grad=False)
        ...
        self.y = nn.Parameter(y.unsqueeze(0).unsqueeze(0).expand_as(self.weight), requires_grad=False)
        self.x = nn.Parameter(x.unsqueeze(0).unsqueeze(0).expand_as(self.weight), requires_grad=False)

    def forward(self, input_image):
        sigma = self.sigma.unsqueeze(-1).unsqueeze(-1).expand_as(self.weight)
        freq = self.freq.unsqueeze(-1).unsqueeze(-1).expand_as(self.weight)
        theta = self.theta.unsqueeze(-1).unsqueeze(-1).expand_as(self.weight)
        psi = self.psi.unsqueeze(-1).unsqueeze(-1).expand_as(self.weight)
